...
local function onCharacterAdded(character)
    if previousPosition and respawning then
        character:WaitForChild("HumanoidRootPart").CFrame = CFrame.new(previousPosition)
        respawning = false
    end
end
...
userInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.R then
        ...
        if player.Character and player.Character:FindFirstChild("Humanoid") then
            respawning = true
            player.Character.Humanoid.Health = 0 -- Reset character
        end
    end
end)
...
local speedBoost = 32 -- Default is 16, so 32 is double speed

humanoid.WalkSpeed = speedBoost

-- Optional: If character dies and respawns, keep the speed
